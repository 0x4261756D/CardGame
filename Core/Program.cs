...
class Program
{
	...
	public static void Main(string[] args)
	{
		...
		PlatformCoreConfig? platformConfig = JsonSerializer.Deserialize(File.ReadAllText(Path.GetFullPath(configPath)), PlatformCoreConfigSerializationContext.Default.PlatformCoreConfig);
			...
			if(s.StartsWith("--"))
			{
				...
				switch(arg)
				{
					case "mode":
						...
						break;
					case "players":
						CoreConfig.PlayerConfig[] players = JsonSerializer.Deserialize(Encoding.UTF8.GetString(Convert.FromBase64String(parameter)), jsonTypeInfo: PlatformCoreConfigSerializationContext.Default.PlayerConfigArray)!;
